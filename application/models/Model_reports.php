...
class Model_reports extends CI_Model
{
    ...
    public function getStoreWiseOrderData2($year, $today_month1, $store)
    {
        if ($year && $store) {
            $sql = "SELECT * FROM orders WHERE paid_status = ? AND store_id = ?";
            $query = $this->db->query($sql, array(1, $store));
            ...
            foreach ($result as $key => $value) {
                # code...
                if (date('M', $result[$key]['date_time']) == $today_month1) {
                    $checkday = date('Y-m-d', $result[$key]['date_time']);
                    if (isset($temparr[$checkday])) {
                        $temparr[$checkday] = $temparr[$checkday] + $result[$key]['gross_amount'];
                    } else {
                        $temparr[$checkday] = $result[$key]['gross_amount'];
                    }
                }
            }
            //var_dump($temparr);
